...
{
    public class GetAllSectorController : ApiController
    {
        [HttpPost]
        public HttpResponseMessage Post([FromBody]Paging paging)
        {
            ...
            using (ClientContext context = new ClientContext(baseurl))
            {
                try
                {
                    ...
                    ReturnSectorObj returnSectorObj = SectorRepository.GetAllSector(context, paging.Position, paging.Items);
                    var response = Request.CreateResponse<ReturnSectorObj>(System.Net.HttpStatusCode.Created, returnSectorObj);
                    return response;
                }
                catch (Exception ex)
                {
                    ReturnSectorObj returnObjcatch = new ReturnSectorObj { Message = ex.Message };
                    var responsecatch = Request.CreateResponse<ReturnSectorObj>(System.Net.HttpStatusCode.Created, returnObjcatch);
