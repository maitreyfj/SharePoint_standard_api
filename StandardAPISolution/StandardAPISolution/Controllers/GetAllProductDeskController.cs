...
{
    public class GetAllProductDeskController : ApiController
    {
        [HttpPost]
        public HttpResponseMessage Post([FromBody]Paging paging)
        {
            ...
            using (ClientContext context = new ClientContext(baseurl))
            {
                try
                {
                    ...
                    ReturnProductDeskObj returnProductDeskObj = ProductDeskRepository.GetAllProductDesk(context, paging.Position, paging.Items);
                    var response = Request.CreateResponse<ReturnProductDeskObj>(System.Net.HttpStatusCode.Created, returnProductDeskObj);
                    return response;
                }
                catch (Exception ex)
                {
                    ReturnProductDeskObj returnObjcatch = new ReturnProductDeskObj { Message = ex.Message };
                    var responsecatch = Request.CreateResponse<ReturnProductDeskObj>(System.Net.HttpStatusCode.Created, returnObjcatch);
