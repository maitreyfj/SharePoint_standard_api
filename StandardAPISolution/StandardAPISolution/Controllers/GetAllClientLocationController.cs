...
{
    public class GetAllClientLocationController : ApiController
    {
        [HttpPost]
        public HttpResponseMessage Post([FromBody]Paging paging)
        {
            ...
            using (ClientContext context = new ClientContext(baseurl))
            {
                try
                {
                    ...
                    ReturnClientLocationObj returnClientLocationObj = ClientLocationRepository.GetAllClientLocation(context, paging.Position, paging.Items);
                    var response = Request.CreateResponse<ReturnClientLocationObj>(System.Net.HttpStatusCode.Created, returnClientLocationObj);
                    return response;
                }
                catch (Exception ex)
                {
                    ReturnClientLocationObj returnObjcatch = new ReturnClientLocationObj { Message = ex.Message };
                    var responsecatch = Request.CreateResponse<ReturnClientLocationObj>(System.Net.HttpStatusCode.Created, returnObjcatch);
