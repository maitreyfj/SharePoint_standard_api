...
{
    public class GetAllCompanyController : ApiController
    {
        [HttpPost]
        public HttpResponseMessage Post([FromBody]Paging paging)
        {
            ...
            using (ClientContext context = new ClientContext(baseurl))
            {
                try
                {
                    ...
                    ReturnCompanyObj returnCompanyObj = CompanyRepository.GetAllCompany(context, paging.Position, paging.Items);
                    var response = Request.CreateResponse<ReturnCompanyObj>(System.Net.HttpStatusCode.Created, returnCompanyObj);
                    return response;
                }
                catch (Exception ex)
                {
                    ReturnCompanyObj returnObjcatch = new ReturnCompanyObj { Message = ex.Message };
                    var responsecatch = Request.CreateResponse<ReturnCompanyObj>(System.Net.HttpStatusCode.Created, returnObjcatch);
