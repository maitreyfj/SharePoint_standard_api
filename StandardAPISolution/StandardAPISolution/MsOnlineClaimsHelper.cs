...
using StandardAPISolution.Controllers;
using System;
...
namespace StandardAPISolution
{
    public class MsOnlineClaimsHelper
    {
        ...
        // Method used to add cookies to CSOM
        public void clientContext_ExecutingWebRequest(object sender, WebRequestEventArgs e,MsoCookies msoCookies)
        {
            //e.WebRequestExecutor.WebRequest.Headers[HttpRequestHeader.Cookie] = "FedAuth=" + _fedAuth + ";rtFa=" + _rtFa;
            e.WebRequestExecutor.WebRequest.CookieContainer = GetCookieContainer_NEW(msoCookies);
            //e.WebRequestExecutor.WebRequest.UserAgent = userAgent;
        }
        ...
        public static CookieContainer GetCookieContainer_NEW(MsoCookies msoCookies)
        {

            if (msoCookies.Expires < DateTime.Now)
            {
                msoCookies.Expires = DateTime.Now.AddDays(1);
            }
            // Get the SAML tokens from SPO STS (via MSO STS) using fed auth passive approach
            MsoCookies cookies = msoCookies;
            ...
            if (!string.IsNullOrEmpty(cookies.FedAuth))
            {
                ...
                if (samlAuth.Expired == true)
                {
                    samlAuth.Expires.AddHours(10);
                }
                return cc;
            }
            return null;
        ...
        // Creates or loads cached cookie container
        public MsoCookies GetCookies_Custom()
        {
            if (_cachedCookieContainer == null || DateTime.Now > _expires)
            {
                // Get the SAML tokens from SPO STS (via MSO STS) using fed auth passive approach
        ...
        private MsoCookies getSamlToken()
        {
            ...
            try
            {
                ...
                //ret.Expires = Convert.ToDateTime(DateTime.Now.AddDays(5));
                ret.Expires = Convert.ToDateTime(DateTime.Now.AddYears(2));
